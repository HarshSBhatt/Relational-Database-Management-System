...
public class Table {
  ...
  public List<Map<String, Object>> getTableValues(String schemaName, String tableName,
    ...
    if (tableFile.exists()) {
      ...
      this.eventLogsWriter.append("Something went wrong! Table does not " + "exist").append("\n");
  ...
  public List<Map<String, Object>> getTableValues(String schemaName, String tableName,
                                                  Map<String, Column> tableColumnsDetails, Set<String> requiredColumnNames, boolean isWithCondition)
      throws Exception {
    String path = Helper.getTablePath(schemaName, tableName);
    File tableFile = new File(path);
  ...
  public Map<String, Column> getTableMetadataMap(String schemaName, String tableName) throws Exception {
  ...
  public boolean hasValidForeignKey(Map<String, Column> tableColumns, String schemaName) throws IOException {
    for (String key : tableColumns.keySet()) {
      Column column = tableColumns.get(key);
      if (column.isForeignKey()) {
        String path = Helper.getTableMetadataPath(schemaName, column.getForeignKeyTable());
        File metadataFile = new File(path);
        if (metadataFile.exists()) {
          BufferedReader br = new BufferedReader(new FileReader(metadataFile));
          String line;
  ...
  public ArrayList<Object> getValuesOfParticularColumn(String schemaName, String tableName, Column column)
      throws IOException {
  ...
  public void printTableValues(String columns, String schemaName, String tableName, String conditions,
                               Map<String, Column> tableColumnsDetails, List<Map<String, Object>> mappedValues, boolean isAllOperation)
      throws Exception {
    List<String> conditionList;

    Map<String, String> conditionMap = new HashMap<>();

    conditionList = Arrays.asList(conditions.trim().split("\\s+"));

    if (conditionList.size() > 1) {
      if (conditions.toUpperCase().contains("AND")) {

        conditionList = Arrays.asList(conditions.trim().split("and"));

        extractConditionAndPrintTable(schemaName, tableName, tableColumnsDetails, conditionList, conditionMap,
            mappedValues, columns, false, isAllOperation);
      } else if (conditions.toUpperCase().contains("OR")) {

        conditionList = Arrays.asList(conditions.trim().split("or"));

        extractConditionAndPrintTable(schemaName, tableName, tableColumnsDetails, conditionList, conditionMap,
            mappedValues, columns, true, isAllOperation);
      } else {
        this.eventLogsWriter.append("Condition mentioned is wrong! Please " + "check your query").append("\n");
        customLock.unlock(schemaName, tableName);
        throw new Exception("Condition mentioned is wrong! Please " + "check your query! Please check your query");
      }
    } else {
      conditionList = Arrays.asList(conditions.trim().split("or"));
  ...
  public void extractConditionAndPrintTable(String schemaName, String tableName,
                                            Map<String, Column> tableColumnsDetails, List<String> conditionList, Map<String, String> conditionMap,
                                            List<Map<String, Object>> mappedValues, String columns, boolean isOrCondition, boolean isAllOperation)
      throws Exception {
    for (String condition : conditionList) {
      ...
      boolean isColExist = Helper.isColumnExist(tableColumnsDetails, conditionColAndVAl.get(0).trim());

      if (!isColExist) {
        this.eventLogsWriter.append("Column: ").append(conditionColAndVAl.get(0).trim())
            .append(" does not exist in " + "table: ").append(tableName).append("! Please check your query")
            .append("\n");
        customLock.unlock(schemaName, tableName);
    ...
    for (Map<String, Object> mappedValueMap : mappedValues) {
      boolean isConditionMatched = conditionChecker(conditionMap, mappedValueMap, isOrCondition);
  ...
  private boolean conditionChecker(Map<String, String> conditionMap, Map<String, Object> mappedValueMap,
  ...
  public void appendColumnInfoToFile(String newColumnName, String[] dataTypeRelatedInfo, StringBuilder fileContent,
                                     FileWriter tableDataWriter) throws IOException {
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      if (!tableIO.isMetadataTableExist(schemaName, tableName)) {
        if (hasValidForeignKey(tableColumns, schemaName)) {
          ...
          String metaDataPath = Helper.getTableMetadataPath(schemaName, tableName);
          String tablePath = Helper.getTablePath(schemaName, tableName);
          ...
          if (metadataFile.createNewFile() && tableFile.createNewFile()) {
            ...
            for (String key : tableColumns.keySet()) {
              ...
              fileContent.append("column_data_type=").append(column.getColumnDataType())
                  .append(AppConstants.DELIMITER_TOKEN);
              ...
              if (column.isPrimaryKey()) {
                if (pk < 1) {
                  ...
                  customLock.unlock(schemaName, tableName);
                  throw new Exception("Table can not have more than one primary " + "key");
                }
              }
              if (column.isAutoIncrement()) {
                if (ai < 1) {
                  ...
                  customLock.unlock(schemaName, tableName);
                  throw new Exception("Table can not have more than one " + "AUTO_INCREMENT field");
                }
              }
              if (column.isForeignKey()) {
                fileContent.append(AppConstants.DELIMITER_TOKEN).append("FK=").append(column.getForeignKeyTable())
                    .append(".").append(column.getForeignKeyColumn());
              }
              fileContent.append("\n");
            }
      ...
      if (tableFile.exists()) {
        ...
        for (int i = 0; i < columnArray.length; i++) {
          ...
          if (primaryKeyColumn != null && columnArray[i].equalsIgnoreCase(primaryKeyColumn.getColumnName())) {
            listOfAvailableValuesInTable = getValuesOfParticularColumn(schemaName, tableName, primaryKeyColumn);
            ...
            listOfAvailableValuesInTable = getValuesOfParticularColumn(schemaName,
                foreignKeyColumn.getForeignKeyTable(), foreignKeyColumn);

            // If value is not present, then we will not insert, because it can not be
            // referenced
            if (!listOfAvailableValuesInTable.contains(columnValue)) {
  ...
  public boolean dropTable(String schemaName, String tableName) throws Exception {
    ...
    if (!schemaFolder.exists()) {
      this.eventLogsWriter.append("Something went wrong! Database: ").append(schemaName).append(" ")
    ...
    if (!tableIO.isTableExist(schemaName, tableName) || !tableIO.isMetadataTableExist(schemaName, tableName)) {
      this.eventLogsWriter.append("Something went wrong! Table: ").append(tableName).append(" ")
          .append("does not exist").append("\n");
      customLock.unlock(schemaName, tableName);
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      BufferedReader br = new BufferedReader(new FileReader(tableMetadataFile));
            ...
            while ((currentTableLine = bufferedReader.readLine()) != null) {
              ...
              if (column.isForeignKey()) {
                ...
                if (foreignKeyColumnName.equals(columnName) && foreignKeyTableName.equals(tableName)) {
                  this.eventLogsWriter.append("Foreign key violation! Table: ").append(tableName)
                      .append(" can not be dropped").append("\n");
                  customLock.unlock(schemaName, tableName);
  ...
  public boolean delete(String schemaName, String tableName, String[] conditionString) throws Exception {
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      if (tableFile.exists()) {
        Map<String, Column> tableColumnsDetails = getTableMetadataMap(schemaName, tableName);

        List<Map<String, Object>> mappedValues = getTableValues(schemaName, tableName, tableColumnsDetails);

        boolean isColExist = Helper.isColumnExist(tableColumnsDetails, conditionColName.trim());

        if (!isColExist) {
          this.eventLogsWriter.append("Column: ").append(conditionColName.trim())
              .append(" does not exist in " + "table: ").append(tableName).append("! Please check your query")
              .append("\n");
          customLock.unlock(schemaName, tableName);
        ...
        conditionMap.put(conditionColName, conditionColValue.trim().replaceAll("[^0-9a-zA-Z]+", ""));

        int rowsAffected = 0;
        for (Map<String, Object> mappedValueMap : mappedValues) {
          boolean isConditionMatched = conditionChecker(conditionMap, mappedValueMap, false);
          if (!isConditionMatched) {
            int i = 0;
        ...
        this.eventLogsWriter.append("Delete operation done on: ").append(tableName).append(" Rows affected: ")
            .append(String.valueOf(rowsAffected)).append("\n");
  ...
  public boolean update(String schemaName, String tableName, String columnsList, String[] conditionString)
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      if (tableFile.exists()) {
        ...
        String[] columnsListString = columnsList.split(",");

        Map<String, String> valueToBeChangedMap = new HashMap<>();

        for (String colAndVal : columnsListString) {
          String[] colAndValString = colAndVal.split("\\s*=\\s*");
          if (Helper.isColumnExist(tableColumnsDetails, colAndValString[0])) {
            valueToBeChangedMap.put(colAndValString[0].trim(),
                colAndValString[1].trim().replaceAll("[^0-9a-zA-Z]+", ""));
          } else {
            this.eventLogsWriter.append("Column: ").append(colAndValString[0].trim())
                .append(" does not exist in " + "table: ").append(tableName).append("! Please check your query")
                .append("\n");
            customLock.unlock(schemaName, tableName);
            throw new Exception("Column: " + colAndValString[0].trim() + " " + "does not exist in table: " + tableName
                + "! Please check your query");
          }
        }
        ...
        for (Map<String, Object> mappedValueMap : mappedValues) {
          ...
          for (String key : mappedValueMap.keySet()) {
            if (isConditionMatched) {
              if (valueToBeChangedMap.containsKey(key)) {
                if (i == mappedValueMap.size() - 1) {
                  fileContent.append(key).append("=").append(valueToBeChangedMap.get(key));
                } else {
                  fileContent.append(key).append("=").append(valueToBeChangedMap.get(key))
                      .append(AppConstants.DELIMITER_TOKEN);
                }
        ...
        this.eventLogsWriter.append("Update operation done on: ").append(tableName).append(" Rows affected: ")
            .append(String.valueOf(finalAffectedRows)).append("\n");
        System.out.println("Rows affected: " + finalAffectedRows);
      } else {
        this.eventLogsWriter.append("Something went wrong! Table does not " + "exist").append("\n");
        System.out.println("Something went wrong! Table does not exist");
        return false;
  ...
  public boolean dropColumn(String schemaName, String tableName, String columnNameToBeDropped)
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      if (tableFile.exists()) {
        ...
        newFileContent = Helper.replaceFileContent(tableFile, columnNameToBeDropped, false);
        ...
        newFileContent = Helper.replaceFileContent(tableMetadataFile, columnNameToBeDropped, true);
  ...
  public boolean changeColumn(String schemaName, String tableName, String oldColumnName, String newColumnName,
    ...
    if (customLock.isLocked(schemaName, tableName)) {
      ...
      if (tableFile.exists()) {
        // Dropping column from metadata table
        newFileContent = Helper.replaceFileContent(tableMetadataFile, oldColumnName, true);
        tableDataWriter = new FileWriter(tableMetadataFile);
        tableDataWriter.write(String.valueOf(newFileContent));
        tableDataWriter.close();
        ...
        StringBuilder updatedValue = Helper.changeColumnNameInFile(tableFile, oldColumnName, newColumnName);
        tableDataWriter = new FileWriter(tableFile);
  ...
  public void fetchTableInfo(String columns, String schemaName, String tableName) throws Exception {
    ...
    if (columns.equalsIgnoreCase("*")) {
      ...
      Set<String> columnNamesInQuery = new LinkedHashSet<>(Arrays.asList(columns.split(",\\s*")));
      ...
      if (columnsNamesInActualTable.containsAll(columnNamesInQuery)) {
        List<Map<String, Object>> mappedValues = getTableValues(schemaName, tableName, tableColumnsDetails,
            columnNamesInQuery, false);

        Helper.printTable(mappedValues);
      } else {
        this.eventLogsWriter.append("One of the column does not exist in " + "table: ").append(tableName)
            .append("! Please check your query").append("\n");
        customLock.unlock(schemaName, tableName);
        throw new Exception("One of the column does not exist in table: " + tableName + "! Please check your query");
      }
    }
  }
  ...
  public void fetchTableInfo(String columns, String schemaName, String tableName, String conditions) throws Exception {
    ...
    if (columns.equalsIgnoreCase("*")) {
      ...
      printTableValues(columns, schemaName, tableName, conditions, tableColumnsDetails, mappedValues, true);
    } else {
      Set<String> columnNamesInQuery = new LinkedHashSet<>(Arrays.asList(columns.split(",\\s*")));
      ...
      if (columnsNamesInActualTable.containsAll(columnNamesInQuery)) {
        List<Map<String, Object>> mappedValues = getTableValues(schemaName, tableName, tableColumnsDetails,
            columnNamesInQuery, true);

        printTableValues(columns, schemaName, tableName, conditions, tableColumnsDetails, mappedValues, false);
